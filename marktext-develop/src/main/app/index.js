...
class App {
  ...
  init () {
    ...
    app.on('window-all-closed', () => {
      // READ-ONLY VIEWER: Don't quit — stay in tray for instant re-open via pipe.
    })
  ...
  ready = () => {
    ...
    // READ-ONLY VIEWER: System tray icon
    if (!this._tray) {
      const iconPath = path.join(__static, 'logo-96px.png')
      ...
      const contextMenu = Menu.buildFromTemplate([
        {
          label: 'Show Window',
          click: () => {
            const active = this._windowManager.getActiveWindow()
            if (active) {
              active.bringToFront()
            } else {
    ...
    // Start named pipe server (includes ready-pool)
  ...
  _openPathList (pathsToOpen, openFilesInSameWindow = false) {
    ...
    // READ-ONLY VIEWER: Always open files in new windows (no tabs)
    const openFilesInNewWindow = true
  ...
  _startPipeServer () {
    ...
    const readyPool = this._readyPool
    ...
    const pipeServer = net.createServer(socket => {
      ...
      socket.on('end', () => {
        ...
        const { preferences } = this._accessor
        const preferredEol = preferences.getPreferredEol()
        const { autoGuessEncoding, trimTrailingNewline } = preferences.getAll()

        // Grab a warm window from pool
        let poolWin = null
        ...
        if (poolWin) {
          loadMarkdownFile(filePath, preferredEol, autoGuessEncoding, trimTrailingNewline).then(doc => {
            console.log('[PERF] pipe-file-loaded: ' + (Date.now() - startTime) + 'ms')
            poolWin.browserWindow.webContents.send('mt::open-new-tab', doc, {}, true)
            poolWin._startHidden = false
            poolWin.browserWindow.show()
            poolWin.browserWindow.focus()
          }).catch(err => {
  ...
  _createWarmWindow () {
    ...
    editor._startHidden = true
    this._windowManager.add(editor)
    if (editor.browserWindow) {
      editor.browserWindow.hide()
    }
    return editor
  }
